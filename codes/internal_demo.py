#!/usr/bin/env python3
"""internal_demo.py — 企业内部知识管理 demo（合成示例数据）

实证 B4 内部使用方案：用框架管企业内部的设备台账 + 合同台账。
- 设备问答（数量/极值/过滤）+ 合同问答（金额/状态）
- 保修/回款提醒（状态过滤）

数据：data_internal/*.csv（合成示例，非真实数据）
"""
import os
import sys
import subprocess
...
DATA = os.path.join(ROOT, "data_internal")
NT = os.path.join(ROOT, "output", "internal.nt")
LEX = os.path.join(ROOT, "config", "lexicon_internal.json")
...
def build():
    ...
    tables = {}
    for t, idc in [("internal_equipment", "id"), ("internal_contracts", "id")]:
