...
0.2.x 为当前版本线（schema 驱动重构 + 检索/评测/上传/前端增强）；0.1.x 为 schema 驱动重构基线；2.9.x 及更早为重构前的能力演进。当前 GitHub release 已更新到 v0.2.0。

## 开源与闭源：同一套工厂本体，两种交付

**本仓库是开源算法层**。工厂本体建模、规则问答、GraphRAG、向量混合检索，全部开源可自部署，换任何行业数据即用。
