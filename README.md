...
**公共词典集（00-03）作为"行业认知基线"固定在此**，仅随方法论升级做必要修正。

- **开源侧**：只维护**代码 + 简单方法论**（loader/吸收/导出机制、README、架构文档）。词典内容**不参与频繁迭代**。
- **服务侧**：词典是服务企业过程中**沉淀的认知资产**，随服务持续增长。服务侧通过 `/api/industry/absorb` 吸收企业词典 → 形成新的行业词典 → `/api/industry/export` 导出积累。
- **不推回开源**：服务侧沉淀的词典（含具体企业痕迹的）**不推回开源仓库**，保持在服务方本地积累，避免开源仓库被服务数据污染，也守住"数据不出厂"红线。
- **开源词典集 = 行业认知基线快照**，新行业词典在服务侧积累到成熟后，可评估是否提炼纯行业知识（去企业痕迹）补入开源基线。
