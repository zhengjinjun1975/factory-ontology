...
工厂本体，是把一家工厂的**数据结构**（MES/ERP/台账）翻译成机器能"看懂"的领域知识图谱：**一张表是什么实体（设备/产品/原料/批次）、字段是它的什么属性（编号/数值/状态/类型）、表之间怎么关联（生产/消耗/交付）**。建好本体，业务人员就能用自然语言问数据，系统在"工厂本体划定的范围内"确定性回答，并给出依据。

核心一句话：**CSV/结构化数据 → 工厂本体建模（schema 驱动）→ 自然语言问答 + 溯源 + GraphRAG 检索**。

**一企业、一行业、一数据**：每个企业绑定一份独立本体，用其所在行业的数据建模，数据与知识在工厂本地闭环——换一个工厂，换一份 schema 和词典，代码不动。
...
v0.2.x 为当前版本线（功能累积升级：检索/评测/上传/前端全面增强）；0.1.x 为 schema 驱动重构基线；2.9.x 及更早为重构前的能力演进。完整变更见 [CHANGELOG.md](CHANGELOG.md)。
...
**本仓库是开源算法层**——工厂本体建模、规则问答、GraphRAG、向量检索，全部开源可自部署，换任何行业数据即用。

在真实的工厂落地中，企业数据与知识的闭环由**配套的闭源服务**完成：
...
开源给出方法与确定性内核，闭源交付完整的服务闭环。这套双轨设计的边界契约见 [docs/双轨边界契约.md](docs/双轨边界契约.md)。
