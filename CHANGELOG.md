...
### 三油科技内部使用方案（B4）
- `docs/三油科技内部使用方案.md`：设备台账/合同台账/技术文档 3 个内部知识库场景 + 落地步骤 + ROI
- `sanyou_demo.py` + `data_sanyou/*.csv`（合成示例）：实证内部知识管理可行（设备数量、客户/状态过滤、金额过滤）
- `config/lexicon_sanyou.json` 设备/合同词典
...
- 与北京三油科技行业交叉佐证：框架天然契合设备台账/质检/零部件溯源
