...
- **CI**：本地全绿；推送后远端 CI `ontology-check` job 通过（本体检查阈值通过）。
  - 远端 CI 综合得分 **99.8 / 100**（`ok=True`、`critical=0`）：`csv` 与 `lexicon` 均为 **100/100**，
    唯一 1 个次要问题是 `[minor](chain) 提示: 决策引擎未提交(CI 纯净检出不含决策层, 属运行时组件)`——
    为**预先存在的运行时组件未提交**信息，与本次 CSV 修复无关（本地因含该未入库文件故本地为 100.0）。
  - `test` job 同样通过（单测 67 通过 + 对照评测），CI 结论 `success`。
